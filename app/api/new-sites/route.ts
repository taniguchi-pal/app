// スプレッドシートで管理する新規現場（案件番号つき）をGoogle Sheets経由で読み書きするプロキシ。
...
export async function GET() {
  ...
  const res = await fetch(`${SHEET_API_URL}?sheet=newsites`, { cache: 'no-store' });
...
export async function POST(request: Request) {
  ...
  const res = await fetch(SHEET_API_URL, {
    ...
    body: JSON.stringify({ ...body, sheet: 'newsites' }),
  });
  const data = await res.json();
  return Response.json(data);
}
