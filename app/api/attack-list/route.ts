// 営業アタックリスト（テレアポ・コンタクト履歴）をGoogle Sheets経由で読み書きするプロキシ。
...
export async function GET() {
  ...
  const res = await fetch(`${SHEET_API_URL}?sheet=attacklist`, { cache: 'no-store' });
...
export async function POST(request: Request) {
  ...
  const res = await fetch(SHEET_API_URL, {
    ...
    body: JSON.stringify({ ...body, sheet: 'attacklist' }),
  });
  const data = await res.json();
  return Response.json(data);
}
